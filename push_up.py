...
class PushUpTrajectoryGenerator(TrajectoryGenerator):
    ...
    def _set_initial_config(self) -> None:
        ...
        self.robot.set_joint("Left_Elbow_Pitch", 1.5)
        self.robot.set_joint("Right_Elbow_Pitch", 1.5)
        self.robot.set_joint("Left_Elbow_Yaw", -1.0)
        self.robot.set_joint("Right_Elbow_Yaw", 1.0)

        T_world_trunk = np.eye(4)
        T_world_trunk[:3, :3] = np.array([
            [np.cos(-1.2), 0.0, -np.sin(-1.2)],
            [0.0, 1.0, 0.0],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hand_spread", type=float, default=0.4, help="Lateral distance between hands (metres).")
    parser.add_argument("--hand_x_offset", type=float, default=0.65, help="Horizontal offset of hands from the center (metres).")
    parser.add_argument("--foot_spread", type=float, default=0.3, help="Lateral distance between feet (metres).")
    parser.add_argument("--push_up_amplitude", type=float, default=0.05, help="Half-amplitude of COM vertical oscillation (metres).")
